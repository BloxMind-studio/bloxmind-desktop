      ...
      studio: {
        ...
        prompt:
          ...
          "ANIMATION: For combat, eating, dance, emote, or reaction requests, author the motion as programmatic KeyframeSequence data (KeyframeSequence > Keyframes > Poses), never as an uploaded asset. The full playbooks also load on demand as the `roblox-animation` and `roblox-animation-runtime` skills — if a skill load reports 'not found', proceed with the reference below and do NOT stop.\n\n" +
           ...
           "- Build via `Instance.new('KeyframeSequence')` with `Keyframe` children (each with a `Time`), and `Pose` children named to match the Motor6D they drive (e.g. a `Pose` named to match the `LeftShoulder`/`Left UpperArm` motor). Set the joint transform on `Pose.CFrame` — there is NO 'Transform' field on a Pose. MANDATORY: immediately after `Instance.new('KeyframeSequence')` set `seq.Priority = Enum.AnimationPriority.Action` so it overrides Idle/Walking — never leave Priority at default.\n" +
           "- R15 joints are `Motor6D`s named like `LeftUpperArm`/`LeftShoulder` (`UpperTorso`, `RightShoulder` no space); R6 uses `Left Shoulder`/`Right Hip`/`Torso` (`Right Shoulder` with space, `Torso` not `UpperTorso`) etc. Detect the rig with `Humanoid.RigType` (`Enum.HumanoidRigType.R15`/`R6`) or `GetRigInfo`, and `GetDescendants` to enumerate joints — never hardcode a joint name without verifying it exists. Pose.Name MUST match the target rig explicitly — `Right Shoulder` for R6 vs `RightShoulder` for R15.\n" +
           "- Play with `Animator:LoadAnimation(animation)` (or `Humanoid:LoadAnimation`) then `Track:Play()`. Use `Enum.AnimationPriority` (e.g. `Action`) and at runtime `Track:GetMarkerReachedSignal('name')` / `Track:AdjustSpeed()`. MANDATORY preview: auto-inject a LocalScript in StarterPlayer.StarterPlayerScripts (src/client → StarterPlayerScripts) that requires your builder (`ReplicatedStorage.ReplicatedShared.Animations.*Builder:Build()` → seq with Priority=Action), does `KeyframeSequenceProvider:RegisterKeyframeSequence(seq)` → `Humanoid:WaitForChild(\"Animator\"):LoadAnimation(anim)` (create Animator if missing) → `track.Priority=Action; track:Play()`, handles `player.Character` already spawned AND `player.CharacterAdded:Connect` for respawn, prints RigType for debug (see R6 video for joint setup). Never place preview in ServerScriptService — it will not run.\n" +
          ...
          "- Do NOT simulate live keyboard/mouse input; drive motion from static animation data. Keep timing readable with anticipation and recovery.\n\n" +
           ...
           "CITY STRICT — QUALITY OVER SPEED (mandatory when request is a city/town/downtown — violation = build failure; do NOT rush to generate quick basic Luau to finish fast, take all extra steps for high-fidelity): 1) Mandatory Smooth Terrain — MUST use workspace.Terrain:FillBlock() or FillRegion() for base terrain (Grass/Dirt/Sand/Asphalt), NEVER flat primitive Part blocks for main ground (paper sheet forbidden), always set workspace.Terrain.Decoration=true for 3D grass blades. Then LAYER: Asphalt ONLY for roads, Concrete for sidewalks/curbs (raised 1.5 studs, 3-4 wide, Stone grey), Grass/Dirt/Sand/LeafyGrass for yards/parks/nature strips — verify ≥3 ground materials. 2) Roads = central Asphalt (12-20 wide) + raised Concrete sidewalks both sides + curb; intersections use Road System/Asphalt Road Pack/Road Intersection Mesh from toolbox or if manual extend Asphalt seamlessly with NO empty gray cutout and NO overlapping curbs + lamps Modern Street Light Pole/Highway Light/Tall Street Lamp every 30-40 studs (never park lanterns). 3) MANDATORY Toolbox search FIRST — before ANY city script call search_toolbox for Modern Street Light Pole/Highway Light/Tall Street Lamp, Building Pack/City Skyscraper Model/Modular Building Mesh (never floating separated box stacks; import full building Models and snap base directly to ground via GetBoundingBox+PivotTo), Road System/Asphalt Road Pack/Road Intersection Mesh, realistic tree/Tree Pack; import usable MeshParts, only manual Parts if search empty. 4) Zero clipping/sinking + neat vegetation: for EVERY Model/MeshPart/tree/building compute exact ground Y via workspace:Raycast() + GetBoundingBox() half-Y and PivotTo(groundY+halfY) so flush on terrain (never floating/underground); trees NEVER on Asphalt/Concrete roads or blocking sidewalks — align neatly inside dedicated grass zones or along nature strips with fixed 18-24 stud grid spacing. 5) SPATIAL GRID & EXPLICIT ROAD OFFSETS (No Random Spacing): Define Blueprint Zones First — RoadZones, SidewalkZones, GreenZones, BuildingPlots before any placement; Calculate Positions Relative to Roads — for road centered at Center_X width W, building plot center MUST be X = Center_X + (W/2) + (BuildingWidth/2) + Clearance (Clearance ≥8); Tree Spacing — trees ONLY inside GreenZones or nature strips offset ≥2 studs from outer edge of sidewalk. 6) SMART INTERSECTION NODE HANDLING (No Curb Blocking): Calculate all road crossings (Intersection_X, Intersection_Z), define clear boundary box (Width_Road1+Sidewalk_Width, Width_Road2+Sidewalk_Width); Break sidewalks into segmented parts that stop at edge of clear zone and resume on other side — NEVER continuous sidewalk through crossing; If using continuous parts apply GeometryService:SubtractAsync() to cut overlap or prioritize Crossroad Intersection / Road Intersection Pack from Toolbox.\n\n" +
